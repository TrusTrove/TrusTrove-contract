...
use trusttrove_fuzz::{attest_invoice, mock_pool_with_asset, InvoiceTestEnv};
...
fn fuzz_invoice_mark_funded(input: MarkFundedInput) {
    ...
    let funded_amount = if expected_funded > 0 {
        expected_funded
    } else {
        1
    };
    ...
    let result = te
        .invoice
        .try_mark_funded(&invoice_id, &pool, &te.usdc_id, &funded_amount);
